...
启动时，RotatorProxy 会先恢复代理池状态并加载所有配置的代理源。来源下载、解析和原生复杂节点构建完成后会立即监听端口；候选节点健康预检查和失效节点静默复查会在后台继续，期间已恢复的可用池照常提供服务。如果设置 `health_check_enabled = false`，则跳过候选节点预检查并直接增量启用所有候选节点。然后把 HTTP 或 SOCKS5 客户端指向配置里的 `listen` 地址，例如 `127.0.0.1:7890`。
...
- `max_concurrent_connections`：同时处理的客户端连接上限，默认 `256`。达到上限后会在 `accept()` 前等待，避免入站连接无限消耗文件描述符。
...
RotatorProxy 不会监控输入目录变化。它只会在启动时和配置的刷新时间重新加载文件与订阅 URL。默认按 `daily_refresh_time` 每天刷新一次；设置 `refresh_interval_seconds` 后改为按固定间隔刷新。启动阶段只等待状态恢复、所有来源加载和原生复杂节点构建，随后立即绑定监听端口；可选批量健康检查与失效节点静默复查在后台完成。后台启动刷新结束后才开始计算下一次定时刷新，避免两轮测活重叠。定时刷新不会停止服务：当前活动代理池会继续处理请求，新的来源会在下载、解析和可选测活完成后增量合并到活动池。刷新不会删除旧活动节点；同 key 节点会去重，只追加新 key 节点。
...
每次来源刷新都会对失效名单里的节点执行后台静默健康检查，即使 `health_check_enabled = false` 也会执行。静默测活使用 `disabled_recheck_concurrency` 控制并发，并在每个节点测完后立即回写内存中的恢复、保留或淘汰结果，避免大量失效节点在启动/刷新期间长期持有连接资源；本轮结果完成后会合并写入一次状态文件，避免并发替换临时文件和反复全量写盘。复杂节点会使用一次性探测适配器，避免 Hysteria QUIC、AnyTLS 空闲会话和 Snell 复用连接在代理池中长期占用文件描述符。如果探测遇到进程或系统级文件描述符耗尽，本轮任务会提前停止，未完成节点不会累计失败。静默测活成功的失效节点会清空失败计数并恢复轮询；静默测活连续失败 3 次的节点会从活动池删除，并在当前进程内记为淘汰，后续刷新即使订阅源仍然包含同 key 节点也不会重新加入。启动阶段如果没有任何已恢复的健康节点，会先按空活动池开始监听，并在后台找到健康候选后增量加入。定时刷新阶段如果新一轮测活没有任何健康节点，现有活动池继续服务。
...
监听器遇到临时 `accept()` 错误（包括 `Too many open files`）时会退避并继续服务，不再直接退出。服务器上仍应为进程配置足够的文件描述符上限；运行中可查看 `/proc/<pid>/limits` 的 `Max open files`，systemd 服务可用 `systemctl show <unit> -p LimitNOFILE` 核对实际生效值。连接很多或服务上限较低时，应同时调低 `max_concurrent_connections` 和 `disabled_recheck_concurrency`。
